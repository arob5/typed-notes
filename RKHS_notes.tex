...
\end{proof}
 ...
 $x_1, \dots, x_n \in \mathcal{X}$, 
 \begin{align*}
 \sum_{i = 1}^{n} \sum_{j = 1}^{n} a_i a_j h(x_i, x_j) &=  \sum_{i = 1}^{n} \sum_{j = 1}^{n} a_i a_j  \langle \phi(x_i), \phi(x_j) \rangle_{\mathcal{F}} \\
 									      &= \langle  \sum_{i = 1}^{n} a_i \phi(x_i), \sum_{i = 1}^{n}  a_i \phi(x_i) \rangle_{\mathcal{F}} \\
									      &> 0
 \end{align*}
 ...
 functions are simply functions that implicitly access inner products in some other space. 
