...
$$
k(x, x^\prime) = \tau^2 \exp\left\{-\frac{1}{2} \sum_{d = 1}^{D} \left(\frac{x_d - x_d^\prime}{\theta_d}\right)^2  \right\}
$$
...
I_{n+1}(x_{n+1}) &= \mathbb{E}_{x \sim \rho}\left[k_{X_n}(x) \right] \\
                 &= \mathbb{E}_{x \sim \rho}\left[k(x) - k_{n+1}^T K_{n+1}^{-1} k_{n+1} \right] \\
                 &= \mathbb{E}_{x \sim \rho}[k(x)] - \mathbb{E}_{x \sim \rho}\left[k_{n+1}^T K_{n+1}^{-1} k_{n+1} \right]
\end{align*}
$$
Note that $K^{-1}_{n+1}$ does not depend on $x$ so is fixed with respect to the expectation; $k_{n+1}$ does depend on $x$ since 
$k_{n+1} = k(X_n, x) = \left(k(x_1, x), \dots, k(x_n,x) \right)^\top$. 
