...
\begin{align}
d\B\state = -\frac{1}{2} \nabla \potential(\B\state) dt + \sigma d\BM \label{Langevin_diffusion_1},
\end{align}
...
\]
However, note that the potential only appears in the diffusion \ref{Langevin_diffusion_1} through its gradient. Therefore, the normalizing constant won't affect the 
resulting SDE. This is great news for developing sampling algorithms, where we typically only know the target density up to a normalizing constant. 
