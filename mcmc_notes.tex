...
\begin{prop}
The systematic scan Gibbs sampler with rotating transition kernels $P_j$ is $\pi$-invariant. 
\end{prop}
...
\[h_{\pi}(x)P(x, y) = h_{\pi}(y)P(y, x) = 0\]
so detailed balance holds in this specific case. Now suppose $x_{-1} = y_{-1}$. We now have 
\begin{align*}
h_{\pi}(x)P(x, y) &= h_{\pi}(x)P(x, y)\pi_{X_1|X_{-1}}(y_1|x_2, \dots, x_d) \\
			 &= h_{\pi}(x)P(x, y) \frac{\pi(y_1, x_2, \dots, x_d)}{\pi(x_2, \dots, x_d)} \\
\end{align*}
...
Our goal is to construct a Markov chain that will converge as quickly as possible to the stationary distribution. In the above proposals, the 
