...
We will also assume that we have observed the true outputs of the simulator $y := (y_1, \dots, y_N)$ when run using design points $x_1, \dots, x_N$.
...
Now, suppose that $\tilde{x}_1, \dots, \tilde{x}_N$ are a set of points in $\mathcal{P}$ and we seek to approximate the values $\tilde{y}_i = f(\tilde{x}_i)$ using the 
...
its mean, etc.). Let $\tilde{Y} := (\tilde{Y}_1, \dots, \tilde{Y}_N)$ and similarly $Y := (\eta(x_1), \dots, \eta(x_N))$, the latter being the random vector of emulator outputs run on the design points. We want 
to consider the \textit{predictive distribution} $\tilde{Y}_1, \dots, \tilde{Y}_N$; that is, of the simulator output at the un-observed parameter values. Thus, we are interested 
in the distribution 
