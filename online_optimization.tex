...
\bigskip
 ...
 \begin{algorithm}[H]
	...
	\For{$t = 0, \dots, T$} {
		$w := \sum_{i \in X} w^{(i)}$ \\
		$x_t := \mathbbm{1}\left\{\sum_{i \in X} \frac{w^{(i)}}{w} u_t^{(i)} \geq \frac{1}{2} \right\}$ \\
...
rate of good experts who just happened to be wrong once or twice. Thus the optimal $\epsilon$ will balance these two factors. 
