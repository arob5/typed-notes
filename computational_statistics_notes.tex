...
quantities of interest). Obtaining the outputs $f(x_1), \dots, f(x_N)$ we now have a few options. A classical approach is to use local optimization 
...
are doing for the Monte Carlo samples. This motivates some of the sampling algorithms described below, which emphasize efficiently filling filling 
the input space $\mathcal{X}$. 

\subsection{Two Sampling Approaches}
We now consider two sampling algorithms to address the computer experiment problems introduced in the previous sections. Note that these algorithms
are also used in many other contexts as well. 
