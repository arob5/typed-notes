...
\[p(\tilde{f}|X, y) = \int p(\tilde{f}|f)p(f|y) df\]
\item Utilize $p(\tilde{f}|X, y)$ from the above step to obtain $p(\tilde{p}|X, y)$. 
\end{enumerate}
...
marginalize over $f$. For step two, the samples from $p(\tilde{f}|X, y)$ can be transformed by applying the inverse logit to obtain samples from $p(\tilde{p}|X, y)$. 
...
In the above examples, $f$ contained function values evaluated at both the training and test points. \textbf{TODO}: change $f$ notation above to not be confusing. 
