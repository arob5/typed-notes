...
\[0 \leq d_H(P, Q) \leq 1\]
Moreover, the $d_H(P, Q) = 0$ if and only if the supports of $P$ and $Q$ are disjoint.  
\end{prop}
...
If there is a measurable set $A \in \mathcal{F}$ satisfying $P(A) > 0$ and $Q(A) > 0$ then the above integral will be strictly positive. 
